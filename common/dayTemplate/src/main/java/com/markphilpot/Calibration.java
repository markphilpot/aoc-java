...
public class Calibration {
    ...
    public static List<String> wordNumbers = List.of(
            "one",
            "two",
            "three",
            "four",
    ...
    public static int getCalibrationValueWithWords(String line) {
        ...
        var earliestWordIndexes = wordNumbers.stream().map(word -> new WordMatch(word, line.indexOf(word))).toList();
        var latestWordIndexes = wordNumbers.stream().map(word -> new WordMatch(word, line.lastIndexOf(word))).toList();

        var earliest = earliestWordIndexes.stream().filter(wm -> wm.index() != -1).min(Comparator.comparing(WordMatch::index));
        var latest = latestWordIndexes.stream().filter(wm -> wm.index() != -1).max(Comparator.comparing(WordMatch::index));

        String first, second;

        if(!earliest.isPresent() && !firstNumber.isPresent()) {
            log.error("First Error :: %s".formatted(line));
        }
        if(!latest.isPresent() && !lastNumber.isPresent()) {
            log.error("Second Error :: %s".formatted(line));
        }

        if(!earliest.isPresent() || (firstNumber.isPresent() && firstNumberIndex < earliest.get().index())) {
            first = firstNumber.get();
        } else {
            first = Integer.toString(wordNumbers.indexOf(earliest.get().word()) + 1);
        }

        if(!latest.isPresent() || (lastNumber.isPresent() && lastNumberIndex > latest.get().index())) {
