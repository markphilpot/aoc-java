...
public class Stacks {
    ...
    private void move(Move move) {
        IntStream.range(0, move.num).forEach(x -> {
    ...
    private void moveBatch(Move move) {
        ...
        IntStream.range(0, tmp.size()).forEach(x -> {
    ...
    public static Stacks parse(InputStream inputStream) {
        try(var scanner = new Scanner(inputStream).useDelimiter("\n")) {
            ...
            initialState.forEach(line -> {
                ...
                IntStream.range(0, num).forEach(x -> {
                    var index = x * 4 + 1;
            ...
            var moveList = moves.stream().map(line -> {
                var s = new Scanner(line).useDelimiter("move (\\d+) from (\\d+) to (\\d+)");
                s.findInLine("move (\\d+) from (\\d+) to (\\d+)");
                MatchResult result = s.match();
                return new Move(Integer.parseInt(result.group(1)), Integer.parseInt(result.group(2)), Integer.parseInt(result.group(3)));
            }).toList();
