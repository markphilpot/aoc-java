...
public class Springs {
    ...
    public static Entry parse(String line, int copies) {
        ...
        var springCopies = IntStream.range(0, copies).boxed().map(i -> spring).collect(Collectors.joining("?"));
        var damageCopies = IntStream.range(0, copies).boxed().map(i -> damage).collect(Collectors.joining(","));
    ...
    public static Stream<Entry> generateCombinations(Entry entry) {
        ...
        var questionIndicies = IntStream.range(0, spring.size()).filter(i -> spring.get(i).equals(State.Q)).boxed().toList();

        var product = Itertools.product(List.of(State.O, State.D), questionIndicies.size());

        var newEntries = product.stream().map(ArrayList::new).map(p -> {
