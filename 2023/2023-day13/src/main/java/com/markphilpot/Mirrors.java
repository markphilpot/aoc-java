...
public class Mirrors {
    ...
    public record Region(List<Mirror> mirrors, Map<Integer, List<Mirror>> mirrorRows, Map<Integer, List<Mirror>> mirrorColumns, int numX, int numY) {}
        ...
        try(var scanner = new Scanner(inputStream)) {
            ...
            while(scanner.hasNextLine()) {
                ...
                if(line.isBlank()) {
                    ...
                    numX = line.length();
                    int finalY = y;
                    regionMirrors.addAll(ParsingUtils.findIndexesOf(line, "#").map(x -> new Mirror(x, finalY)).toList());
                    y++;
                }
            }
    ...
    public static Optional<Integer> findRowReflectionIndex(Region region) {
        // For now going to assume regions are defined as a single refection (rather than needing to find the "largest")
...
//                log.info("Found initial Col Reflection %d %d".formatted(x, x+1));
    ...
    public static Integer getRegionScore(Optional<Integer> rowIndex, Optional<Integer> colIndex) {
        ...
        return rowIndex.map(integer -> (integer + 1) * 100).orElseGet(() -> (colIndex.orElseThrow(IllegalStateException::new) + 1));
    }
}
