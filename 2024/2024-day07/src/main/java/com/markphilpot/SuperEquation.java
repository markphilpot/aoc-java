...
public record SuperEquation(Long result, List<Long> operands) {
  private static final Logger log = LogManager.getLogger(SuperEquation.class);

  public static SuperEquation parse(String line) {
    var els = line.split(": ");
    var result = Long.parseLong(els[0]);
    var operands = ParsingUtils.readLongs(els[1]).toList();
    return new SuperEquation(result, operands);
  }
    ...
    for(var opSet : operators) {
      ...
      for (int i = 1; i < operands.size(); i++) {
        ...
        x = switch (op) {
          case "+" -> x + next;
          case "*" -> x * next;
          case "||" -> Long.parseLong("%s%s".formatted(x, next));
